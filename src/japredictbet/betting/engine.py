...
def _validate_lambda(lambda_: float, context: str = "") -> None:
    ...
    if lambda_ == 0:
        msg = "Lambda is 0 (no corners expected)"
        if context:
            msg = f"[{context}] {msg}"
        logger.warning(msg)
...
def evaluate_bet(
    lambda_: float, line: float, odds: float, bet_type: str = "over", edge_threshold: float = 0.05
) -> dict:
...
def evaluate_match(lambda_home: float, lambda_away: float, odds_data: dict, edge_threshold: float = 0.05):
...
def evaluate_result(real_value: int, line: float, bet_type: str) -> bool | None:
...
def compute_profit(result: bool | None, odds: float, stake: float = 1.0) -> float:
...
def report_consensus(
    ...
    # 4. Decisao
    status = "VALUE BET CONFIRMADA" if agreement >= consensus_threshold else "ABSTENCAO (INSEGURA)"
...
class ConsensusEngine:
    ...
    def evaluate_with_consensus(
        ...
        if model_weights is not None and len(model_weights) == total_models:
            weighted_positive = sum(w * v for w, v in zip(model_weights, model_votes, strict=False))
        ...
        consensus_label = f"Consenso: {positive_votes}/{total_models} - {agreement:.0%} | Status: {decision_status}"
...
def _extract_lambda_total(prediction: dict) -> float:
    ...
    if "lambda_total" in prediction:
        ...
        raise ValueError("Each prediction must contain either 'lambda_total' or 'lambda_home' and 'lambda_away'.")
