# JA PREDICT BET — ROADMAP (REVISÃO 09-MAI-2026)

**Data da Revisão:** 09 de Maio, 2026
...
**Itens pendentes:** 53 total (54 anteriores + ODDS.1, menos P2.C5 e NEW.3 removidos por revisão)
...
> **Revisão 09-MAI-2026:** novo item crítico `ODDS.1` priorizado antes de performance/notificações. A matriz de odds deve ser aplicada antes da LLM para reduzir tokens e impedir que odds inelegíveis contaminem a análise; o hardening pós-LLM permanece como defesa final.

---

## 🔴 P1 — Correções Críticas (Auditoria 07/09-MAI-2026) (3 itens — ~5-8h)
...
- [ ] **ODDS.1 — The Bouncer V2: pré-filtro determinístico de odds/jogos antes da LLM** ⚠️
  - **Tipo:** Correção Crítica / Economia de Tokens / Gestão de Risco.
  - **Problema:** A auditoria do `logs/shadow_bets.log` mostrou que seleções em `ZONA MORTA` (`<1.25`) e `ZONA DE COMPOSIÇÃO` (`1.25-1.59`) chegaram ao Gatekeeper e foram aprovadas pelo LLM em alguns casos. O pós-LLM já foi endurecido em 09-MAI-2026, mas isso deve ser a última defesa, não a primeira barreira. Odds matematicamente inelegíveis consomem contexto, confundem a resposta e podem ocupar o limite diário antes de entradas válidas.
  - **Estratégia em 3 camadas:**
    1. `scraper_filter`: remove mercados não rastreados, remove odds `<1.25`, reduz linhas redundantes antes de salvar o snapshot.
    2. `llm_candidate_builder`: monta payload compacto, marca `zone`, `stake_allowed`, `best_pick_allowed`, e decide se o jogo merece chamada LLM.
    3. `gatekeeper_post_guard`: mantém a trava rígida pós-LLM já implementada, corrige respostas inválidas e impede `best_pick` fora da faixa permitida. Deve permanecer coberta por testes de regressão.
  - **Ação:**
    1. Criar módulo/função de candidatura pré-LLM (ex.: `src/japredictbet/odds/pre_llm_filter.py`) que opere por **seleção**, não apenas por evento.
    2. Integrar esse filtro ao snapshot do scraper (`scripts/superbet_scraper.py::_apply_snapshot_filter`) para remover seleções inelegíveis antes de salvar JSON.
    3. Integrar o mesmo filtro ao Gatekeeper/pipeline (`GatekeeperAgent._apply_pre_filter` ou etapa anterior equivalente) para que o LLM receba apenas candidatos já normalizados.
    4. Remover seleções com `odd < 1.25` do payload enviado ao Gatekeeper; registrar contagem/motivo em log estruturado.
    5. Manter `1.25 <= odd < 1.60` apenas quando modo composição estiver habilitado; marcar como `COMPOSITION_ONLY`, `stake_allowed=false`, `best_pick_allowed=false`.
    6. Manter `1.60 <= odd <= 2.20` como `SINGLE`; manter `odd > 2.20` como `VARIANCE_SINGLE` com `max_stake=0.5`.
    7. Aplicar whitelist de mercados antes da LLM: 1x2, BTTS, Over/Under Gols, Escanteios e 1º Tempo quando suportado. Remover handicap, props, placar correto e mercados exóticos.
    8. Reduzir linhas redundantes por mercado (ex.: no máximo 1-2 linhas por direção, priorizando Zona Alvo e uma linha de composição quando relevante).
    9. Só chamar LLM quando restar pelo menos 1 seleção `SINGLE`/`VARIANCE_SINGLE`, ou 2+ `COMPOSITION_ONLY` se o modo composição estiver ativo.
    10. Adicionar testes unitários cobrindo: descarte `<1.25`, builder sem stake/best_pick, variância com `max_stake=0.5`, mercado fora da whitelist e evento sem candidato.
  - **Impacto:** Reduz custo de tokens, melhora disciplina operacional e evita que ruído de odds baixas contamine a análise contextual.
  - **Pré-requisito para:** Paralelismo/async (FASE 2/6), Telegram notifier e qualquer automação operacional.
  - **Esforço:** ~1-2h.
...
```
🔴 P1 (⚠️ Correções)       ██████████████████████  3 itens  |  5-8h    ← ODDS.1 + News Context + Test Mocks
FASE 1 (🧹 Limpeza)        ██████░░░░░░░░░░░░░░░░  5 itens  |  3-4h    ← Dead code + boundary + pyproject + faxina de arquivos
...
FASE 6 (⚡ Performance)     ███████░░░░░░░░░░░░░░░  3 itens  |  4-7d    ← Async completo + Notifier + Cache API
...
## 🧹 FASE 1 — Limpeza Imediata (5 itens — ~3-4h) ✅ CONCLUÍDA 09-MAI-2026
...
- [x] **P2.C1/C6 — Remover `add_rolling_features()` + teste associado**
...
- [x] **P2.C2 — Resolver boundary `probability/` vs `betting/engine.py`**
...
- [x] **Completar `pyproject.toml`**
  ...
  - **Ação:** Adicionar `description`, `authors`, `classifiers`, `keywords`, `license`, `requires-python` (>=3.11), `dependencies` (principais: pandas, numpy, scikit-learn, xgboost, lightgbm, httpx, openai, python-dotenv, pyyaml). Entry points removidos — uso via `python scripts/` mantido.
  - **Esforço:** ~30min.

- [x] **NEW.1 — Remover `config_backup.yml`**
  - **Arquivo removido.** Era referência stale de config antiga, já constava como removido no `COMPLETION_HISTORY.md`.

- [x] **NEW.2 — Limpar scripts de investigação `_*.py` remanescentes**
  - `_cross_ref_2024.py` → removido (obsoleto).
  - `_discover_tournaments.py` → renomeado para [`discover_tournaments.py`](scripts/discover_tournaments.py).
  - `_scrape_superbet_playwright.py` → renomeado para [`scrape_superbet_playwright.py`](scripts/scrape_superbet_playwright.py).

## ⚡ FASE 2 — Quick Wins de Performance e Contexto (4 itens — ~5-10h)

> **Revisão 09-MAI-2026:** Ordem reordenada após auditoria de código. AUDIT.3 sobe de prioridade porque o Bouncer V2 (ODDS.1) estreita o funil de candidatos — cada jogo perdido por mismatch custa mais. P3.ENG cai para último: paralelismo com dados ruins é perigoso; melhor garantir qualidade antes de acelerar. Notas de segurança (`threading.Lock`, rate limit) adicionadas.

**Objetivo:** Baixo esforço, alto impacto. Recuperar jogos descartados por divergência de nomes, preencher gaps críticos de contexto que já têm dados disponíveis, e reduzir latência do pipeline.

**Ordem de execução recomendada:**

```
1. AUDIT.3 (Fuzzy Match)  →  melhora matching que ENR.3/ENR.4 dependem
2. ENR.3 (Árbitro)        →  $0 custo API, extração trivial, captura team.id
3. ENR.4 (H2H)            →  depende de team.id capturado em ENR.3
4. P3.ENG (ThreadPool)    →  paralelismo só depois de qualidade garantida
```

- [ ] **AUDIT.3 — Fuzzy Matching de Nomes de Times (rapidfuzz)** ⚠️ **PRIORIDADE ELEVADA**
  - **Tipo:** Correção de Perda de Dados — Alta Prioridade.
  - **Justificativa de prioridade:** O Bouncer V2 (ODDS.1) reduz o número de candidatos que chegam ao LLM. Com o funil mais estreito, perder ~10-15% de jogos elegíveis por divergência de substring entre Superbet e API-Football é **mais custoso** do que antes — cada jogo que sobrevive ao pré-filtro tem maior probabilidade de ser EV+.
  - **Problema:** Discrepâncias entre nomes na Superbet (ex: "Internacional RS") e na API-Football (ex: "Internacional") causam descarte silencioso de jogos elegíveis. Três funções usam matching por substring (`in`), frágil com variações de nome (FC vs F.C., siglas de estado, acentos):
    - [`_match_superbet_odds()`](src/japredictbet/data/context_collector.py:712) — `home_l in sb_home_l or sb_home_l in home_l`
    - [`enrich_pre_match_contexts()`](src/japredictbet/data/context_collector.py:624) — mesmo padrão
    - [`_find_standing()`](src/japredictbet/data/context_collector.py:957) — `lower in entry.team.lower()`
  - **Estimativa de impacto:** ~10-15% dos jogos scrapingdos são descartados por mismatch de nome de time.
  - **Ação:**
    1. Adicionar `rapidfuzz>=3.0.0` (mais rápida que `thefuzz`) ao [`requirements.txt`](requirements.txt) e ao [`pyproject.toml`](pyproject.toml) (`[project] dependencies` + `[project.optional-dependencies].dev`).
    2. Criar função `_fuzzy_match_teams(name_a: str, name_b: str, threshold: float = 0.85) -> bool` usando `rapidfuzz.fuzz.token_sort_ratio()` (robusto a inversão de palavras como "Manchester City" vs "City Manchester") com fallback `partial_ratio()` para casos extremos.
    3. Substituir matching por substring nos 3 locais afetados pela função fuzzy.
    4. Se confiança > 85% e o par não estiver em [`superbet_teams.json`](data/mapping/superbet_teams.json), fazer append automático ao mapeamento (auto-healing).
  - **Impacto:** Recuperação de ~10-15% de jogos que seriam descartados. Redução de manutenção manual do `superbet_teams.json`.
  - **Esforço:** ~1-2h.
  - **Pré-requisito para:** ENR.3 e ENR.4 (melhor matching → mais jogos enriquecidos com árbitro e H2H). SH4 (preenchimento automático acelera o mapeamento manual).

- [ ] **ENR.3 — Árbitro no MatchContext**
  - **Tipo:** Correção de Gap Operacional.
  - **Problema:** O dataclass [`RefereeInfo`](src/japredictbet/data/context_collector.py:63) já existe mas **nunca é populado** — 0 ocorrências de extração de `referee` em todo o código. O campo `fixture.referee` **JÁ VEM** no response do endpoint `fixtures` da API-Football — nenhuma chamada extra necessária. O [`to_llm_context()`](src/japredictbet/data/context_collector.py:190-191) já serializa `RefereeInfo` se não for `None`.
  - **Ação:** Extrair `fixture_info.get("referee")` (ou `match_fix.get("fixture", {}).get("referee")` no caso de `enrich_pre_match_contexts`) em 2 locais:
    1. [`collect_upcoming()`](src/japredictbet/data/context_collector.py:483) — após `fixture_info.get("date")`
    2. [`enrich_pre_match_contexts()`](src/japredictbet/data/context_collector.py:617) — após encontrar `match_fix`
    Popular `ctx.referee = RefereeInfo(name=referee_name)` quando o campo não for `None`/vazio.
  - **Captura de team.id:** Aproveitar para capturar `teams.home.id` e `teams.away.id` (já disponíveis no mesmo response) — necessário para ENR.4.
  ...
  - **Pré-requisito para:** ENR.4 (team.id capturado aqui é usado no endpoint `fixtures/headtohead`).
...
- [ ] **ENR.4 — H2H Recente no MatchContext**
  ...
  - **Ação:**
    1. Adicionar `get_h2h(team_a_id: int, team_b_id: int) -> list[dict[str, Any]]` ao [`ApiFootballClient`](src/japredictbet/data/context_collector.py:209-336) usando endpoint `fixtures/headtohead` (parâmetro `h2h` com string `"{id_a}-{id_b}"`).
    2. Adicionar campo `h2h: list[dict[str, Any]] | None = None` ao [`MatchContext`](src/japredictbet/data/context_collector.py:122).
    3. Serializar resumo dos últimos 3 confrontos (placar, gols, data) no [`to_llm_context()`](src/japredictbet/data/context_collector.py:144).
    4. Popular `h2h` em `collect_upcoming()` e `enrich_pre_match_contexts()`.
  - **⚠️ Restrição de Rate Limit:** API-Football free tier tem **100 req/dia**. Com 30 jogos, H2H consumiria 30 chamadas. **Estratégia:** só chamar `get_h2h()` para jogos que passaram no Bouncer V2 (`build_llm_candidates().should_call_llm == True`). Isso reduz chamadas para ~5-8/dia, deixando folga para fixtures + lineups + standings.
  - **Impacto:** Análise 1x2 e Over/Under com histórico real de confronto direto.
  - **Esforço:** ~2-4h.
  - **Dependência:** ENR.3 (precisa de `team.id` capturado durante extração de árbitro).

- [ ] **P3.ENG quick-win — `ThreadPoolExecutor` no loop de avaliação**
  - **Tipo:** Melhoria de Performance.
  - **Situação atual:** O loop `for match_ctx in matches` em [`gatekeeper_live_pipeline.py`](src/japredictbet/pipeline/gatekeeper_live_pipeline.py:238) é sequencial — 30 jogos × ~5s cada = ~150s. Confirmado: 0 ocorrências de `concurrent.futures` no código.
  - **Ação:** Envolver o loop com `concurrent.futures.ThreadPoolExecutor(max_workers=8)`, processando `_evaluate_single_match()` em paralelo. Cada worker chama o LLM independentemente (`OpenAI` client é thread-safe por padrão).
  - **⚠️ Segurança de thread:** `approved_count` e `max_entries` exigem `threading.Lock` para o contador de entradas aprovadas. O Bouncer V2 (`pre_llm_filter`) é stateless — seguro para uso concorrente.
  - **Impacto:** Latência ~150s → ~25-30s (com Bouncer V2 reduzindo candidatos, o ganho é ligeiramente menor que os ~20s originais, mas ainda ~5x mais rápido).
  - **Esforço:** ~30min-1h.
  - **Pré-requisito para:** P3.ENG completo (migração asyncio na Fase 6).
  - **Executar por último:** Paralelismo com dados ruins é perigoso. Garantir que AUDIT.3 + ENR.3 + ENR.4 estejam estáveis antes de acelerar.
...
## ⚡ FASE 6 — Performance & Notificação (3 itens — ~4-7 dias)
