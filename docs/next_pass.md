...
**Status Geral:** P0 ✅ | P0-FIX ✅ | P1 ✅ | Onda 1 ✅ | Onda 2 ✅ | Onda 3 ✅ | Onda 4 parcial | P3-ARCH ✅ | P2-REFACTOR ✅ | CI Pipeline ✅ | ENR.1 ✅ | FASE 0 ✅ | SCRAPER.1 ✅ | SCRAPER.2 ✅ | ODDS.1 ✅ | FASE 1 ✅ — **254/254 testes** (21 arquivos). 106 features. 30 modelos treinados. Scraper migrado para REST by-date (20 TIDs rastreados de 19 ligas). Playwright com multi-scroll. Mapeamento `list[int]` para TIDs múltiplos. The Bouncer V2 (pré-filtro de odds) operacional.
**Histórico Completo:** [`COMPLETION_HISTORY.md`](COMPLETION_HISTORY.md)
**Itens pendentes:** 43 total — reorganizados 10-MAI-2026: AUDIT.4 (Telegram) subiu para FASE 2, NEW.7 (Health Check) subiu para FASE 3
...
> **Revisão 10-MAI-2026:** Backlog reordenado. AUDIT.4 (Telegram Notifier) promovido da FASE 6 → FASE 2: quick-win standalone de ~1-2h que entrega operação remota imediata. NEW.7 (Health Check) promovido da FASE 5 → FASE 3: pré-condição de confiabilidade antes de pipelines pesados. Nota: P2.D1 (tratamento de erros) deve ser concluído antes de ENR.4 (H2H) e P3.ENG (ThreadPool) — paralelismo sem error handling amplifica falhas.
...
- **Alerta:** `odds/collector.py` usa `requests` (bloqueante) enquanto todo o resto do projeto adotou `httpx`. Migrar na FASE 3 (NEW.4).
...
- **✅ (Resolvido 09-MAI-2026 — ODDS.1 + FASE 1):** The Bouncer V2 operacional. `config_backup.yml` removido. `add_rolling_features()` removido. Poisson migrado para `probability/poisson.py`. `pyproject.toml` completo.
...
```
FASE 2 (⚡ Quick Wins)      ██████░░░░░░░░░░░░░░░░  5 itens  |  6-12h   ← Fuzzy + Árbitro + H2H + ThreadPool + Telegram
FASE 3 (🏗️ Fundação)       ██████░░░░░░░░░░░░░░░░  6 itens  |  3-4d    ← Logging + Erros + httpx + retry + Health Check
FASE 4 (🧪 Testes)         ██████████████████████  10 itens |  3-5d    ← +5 módulos, cobertura ~55%→70%
FASE 5 (🎛️ Produto)        ████████████░░░░░░░░░░  6 itens  |  4-7d    ← SH4 + Dashboards + Auto-healing + CLI
FASE 6 (⚡ Performance)     ███████░░░░░░░░░░░░░░░  2 itens  |  3-5d    ← Async completo + Cache API
FASE 7 (🧠 Contexto)       ██████████░░░░░░░░░░░░░  4 itens  |  1-2sem  ← Memória Leve + RAG + Scout + xG Anchor
FASE 8 (🤖 Avançado)       ██████████░░░░░░░░░░░░  4 itens  |  2-4sem  ← LLM Consensus + Auditor + Telegram Bot + Docker
...
## ⚡ FASE 2 — Quick Wins de Performance e Contexto (5 itens — ~6-12h)

> **Revisão 10-MAI-2026:** AUDIT.4 (Telegram Notifier) promovido da FASE 6 — quick-win standalone de ~1-2h com zero dependências, entrega operação remota imediata. Ordem mantida: qualidade de dados (Fuzzy → Árbitro → H2H) antes de performance (ThreadPool). Nota: P2.D1 (tratamento de erros na FASE 3) deve idealmente ser concluído antes de ENR.4 e P3.ENG — H2H faz chamadas de rede que podem falhar, ThreadPool amplifica falhas.

**Objetivo:** Baixo esforço, alto impacto. Recuperar jogos descartados por divergência de nomes, preencher gaps críticos de contexto, reduzir latência do pipeline, e habilitar operação remota imediata.
...
5. AUDIT.4 (Telegram)     →  standalone, pode ser feito em paralelo com qualquer item acima
```
...
## 🏗️ FASE 3 — Fundação de Observabilidade (6 itens — ~3-4 dias)
...
## 🎛️ FASE 5 — Produto & Dados (6 itens — ~4-7 dias)

**Objetivo:** Completar integração multi-liga (SH4), melhorar UX (menu bootstrap, dashboards), automatizar manutenção (auto-healing) e unificar interface CLI.
...
## ⚡ FASE 6 — Performance & Cache (2 itens — ~3-5 dias)

**Objetivo:** Otimizações que reduzem tempo de execução e custo de tokens. Aproveita ganhos da Fase 2 (ThreadPoolExecutor) e avança para solução assíncrona completa com cache inteligente de API.
