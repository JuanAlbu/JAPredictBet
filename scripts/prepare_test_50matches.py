...
if len(subset_180) < 60:
    ...
    all_recent = pd.concat([previous_season.tail(100), subset_180]).sort_values("Date").reset_index(drop=True)
...
if len(subset_180) >= 80:
    ...
    sample_50 = (
        pd.concat([first_part, second_part.sample(n=min(50, len(second_part)), random_state=42)])
        .sort_values("Date")
        .reset_index(drop=True)
    )
    print("✅ Sample de 50 jogos criada (com contexto histórico)")
else:
...
if not Path(config_backup).exists():
    ...
    print("✅ Config backup criado")

# Ler config e modificar para usar dataset de teste
with open(config_original) as f:
    config_content = f.read()

config_test = config_content.replace(
    'raw_path: "data/raw/dataset.csv"', 'raw_path: "data/raw/dataset_test_50matches.csv"'
)
